...
__all__ = ['SEMXResNetB', 'semxresnetb18', 'semxresnetb34', 'semxresnetb50', 'semxresnetb101', 'semxresnetb152']
...
class SEMXResNetB(nn.Sequential):
...
def semxresnetb(expansion, n_layers, name, pretrained=False, **kwargs):
    model = SEMXResNetB(expansion, n_layers, **kwargs)
    if pretrained: 
        #model.load_state_dict(model_zoo.load_url(model_urls[name]))
        print("No pretrained yet for SEMXResNetB")
    return model
...
for n,e,l in [
    ...
    name = f'semxresnetb{n}'
    setattr(me, name, partial(semxresnetb, expansion=e, n_layers=l, name=name))
